...
namespace svg
{
    ...
    type_code encode (string const& type_string) {
        ...
        else if (type_string == "g") return group;
        else return other;
    }
    ...
    void readSVG(const string& svg_file, Point& dimensions, vector<SVGElement *>& svg_elements)
    {
        ...
        for(XMLElement* child = xml_elem->FirstChildElement(); child != NULL; child = child->NextSiblingElement()) {
            parseElement(child, shapes, pos, delimiter);
        }
    ...
    void parseElement(tinyxml2::XMLElement *child, std::vector<svg::SVGElement *> &shapes, size_t &pos, std::string &delimiter)
    {
        ...
        case rect:
            ...
            if (child->Attribute("transform") != NULL)
            {
                ...
                if (transform.find("translate") != string::npos)
                {
                    translate = {stoi(transform.substr(transform.find("(") + 1, transform.find_first_of(" ,") - transform.find("(") - 1)), stoi(transform.substr(transform.find_first_of(" ,") + 1, transform.find(")") - transform.find_first_of(" ,") - 1))};
                    for (Point &p : c_points)
                    {
                        p = p.translate(translate);
                    }
